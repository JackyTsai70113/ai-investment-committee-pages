...
const DISPLAY_TRANSLATIONS = [
  ...
  ["所有市場風險部位都必須歸零", "市場風險部位不因這項提示自動歸零"],
  ["政策已將保證模式下的風險資產上限設為 0%，並由程式驗證，不依賴模型自行遵守。", "短期報酬不作保證，資料提示不會自動將風險資產歸零。"],
  ...
  ["研究建議維持全部資金為現金，不代表交易已執行；在短窗統計、事件對齊與跳空檢查未完成前，不新增方向性曝險。", "目前快照的研究建議為現金配置，不代表交易已執行；資料缺口僅作警示，重新執行委員會後更新方向。"],
